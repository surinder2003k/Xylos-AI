"use client";

import { Twitter, Facebook, Share2 } from "lucide-react";
import { useToast } from "@/components/ui/toast";

interface ShareButtonsProps {
  title: string;
  excerpt: string;
  slug: string;
}

export function ShareButtons({ title, excerpt, slug }: ShareButtonsProps) {
  const { showToast } = useToast();
  const url = `https://xylosai.vercel.app/blog/${slug}`;

  const handleShare = () => {
    if (navigator.share) {
      navigator.share({ title, text: excerpt, url }).catch(() => {});
    } else {
      navigator.clipboard.writeText(url);
      showToast("Link copied to clipboard!", "success");
    }
  };

  return (
    <div className="hidden md:flex items-center gap-4 pr-6 mr-2" style={{ borderRight: '1px solid rgba(190, 184, 170, 0.2)' }}>
      <a 
        href={`https://twitter.com/intent/tweet?url=${url}&text=${encodeURIComponent(title)}`} 
        target="_blank" 
        rel="noopener noreferrer" 
        className="text-gray-400 hover:text-[#36b7b0] transition-colors"
      >
        <Twitter className="w-4 h-4" />
      </a>
      <a 
        href={`https://www.facebook.com/sharer/sharer.php?u=${url}`} 
        target="_blank" 
        rel="noopener noreferrer" 
        className="text-gray-400 hover:text-[#36b7b0] transition-colors"
      >
        <Facebook className="w-4 h-4" />
      </a>
      <button 
        onClick={handleShare} 
        className="text-gray-400 hover:text-[#36b7b0] transition-colors"
      >
        <Share2 className="w-4 h-4" />
      </button>
    </div>
  );
}
